...
$totalMembers = $db->query("SELECT COUNT(*) FROM legislators where chamber = 'House of reps' AND is_archived = FALSE")->fetchColumn();
$totalFemale = $db->query("SELECT COUNT(*) FROM legislators where chamber = 'House of reps' AND gender = ' Female' AND is_archived = FALSE")->fetchColumn();
$femalePercentage = ($totalMembers > 0) ? round(($totalFemale / $totalMembers) * 100, 2) : 0;
$totalMale = $db->query("SELECT COUNT(*) FROM legislators where chamber = 'House of reps' AND gender = 'Male' AND is_archived = FALSE")->fetchColumn();
$malePercentage = ($totalMembers > 0) ? round(($totalMale / $totalMembers) * 100, 2) : 0;
$totalYoungMembers = $db->query("SELECT COUNT(*) FROM legislators where chamber = 'House of reps' AND is_archived = FALSE AND TIMESTAMPDIFF(YEAR, dob, CURDATE()) < 40")->fetchColumn();
...
$members = $db->query("SELECT id, name, gender, dob, state, constituency, position FROM legislators where chamber = 'House of reps' AND is_archived = FALSE")->fetchAll();
?>
