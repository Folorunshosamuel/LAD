document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("fetch_state_assembly_data.php?members=true")
        ...
        .then(fetchedData => {
            ...
            updateStatsAndCharts(data); // Load stats & charts initially
        })
    ...
    // Update Stats Cards & Charts
    function updateStatsAndCharts(filteredData) {
    ...
    function renderCharts(stateData) {
        ...
        stateData.forEach(item => {
            parties[item.party] = (parties[item.party] || 0) + 1;
        });
...
function calculateAge(dob) {
    if (!dob || dob.trim() === "") return 100; // If DOB is missing, return a high age
